...
//这里改变全局变量  修改  起始日期  或者是结束日期  的地方
if(willBeChangVar){ 
	//alert(willBeChangVar); 
	changIt(obj.value);
}
//alert(obj.value); // wengjiang  就是在这里 来处理
this.hide();
};
...
/***********这里只是附加   为了适应这个工程 而做的   公用方法   这个不会影响上面的程序 里面会判断 willBeChangVar 不为空的时候才会改变这个值   *********/ 
var willBeChangVar=null; 
function changIt(date){
	 if  (willBeChangVar=="globleStrStartTime"){
		 globleStrStartTime=date+" 00:00:00";
		// alert(globleStrStartTime);
	 }else if(willBeChangVar=="globleStrEndTime"){
		 globleStrEndTime=date+" 00:00:00";
	 }
	 refreshTime();
}
