...
public class DevicesStatisService {
	...
	public List<Object[]> getSubOrgDeviceNum(Integer OrgId) {
		...
		for(int i=0;i<OrgInfoList.size();i++){
			...
			String Condition = "in ("+strOrganizeSubOrgList.substring(0,strOrganizeSubOrgList.length()-1)+")";
			//System.out.println("Condition:"+Condition);
			Long DeviceDomNum = this.devicebaseinfoDao.getDeviceNumByOrgId(4, (Integer)(OrgInfoList.get(i)[0]), Condition);
	...
	public List<Object[]> getSubOrgDeviceNumByCondition(Integer Item, Integer OrgId) {
		System.out.println("getSubOrgDeviceNumByCondition is here!");
		List<Object[]> SubOrgDeviceNum = new ArrayList();
		List<Object[]> OrgInfoList = this.clientGroupDAO.getOrglists(OrgId);
		System.out.println("OrgInfoList.size:"+OrgInfoList.size());
		for(int i=0;i<OrgInfoList.size();i++){
			...
			String Condition = "in ("+strOrganizeSubOrgList.substring(0,strOrganizeSubOrgList.length()-1)+")";
			//System.out.println("Condition:"+Condition);
			Long RegistedNum,RegAndDomNum;
			if(Item == 0){
			    RegistedNum = this.devicebaseinfoDao.getDeviceNumByOrgId(5, (Integer)(OrgInfoList.get(i)[0]), Condition);
			    RegAndDomNum = this.devicebaseinfoDao.getDeviceNumByOrgId(6, (Integer)(OrgInfoList.get(i)[0]), Condition);
			}
			else if(Item == 1){
			    RegistedNum = this.devicebaseinfoDao.getDeviceNumByOrgId(7, (Integer)(OrgInfoList.get(i)[0]), Condition);
			    RegAndDomNum = this.devicebaseinfoDao.getDeviceNumByOrgId(8, (Integer)(OrgInfoList.get(i)[0]), Condition);
			}
			else{
			    RegistedNum = this.devicebaseinfoDao.getDeviceNumByOrgId(9, (Integer)(OrgInfoList.get(i)[0]), Condition);
			    RegAndDomNum = this.devicebaseinfoDao.getDeviceNumByOrgId(10, (Integer)(OrgInfoList.get(i)[0]), Condition);
			}
			Object[] SubOrgInfo = {(String)(OrgInfoList.get(i)[1]), RegistedNum, RegAndDomNum};
			System.out.println((String)SubOrgInfo[0]+(Long)SubOrgInfo[1]);
			SubOrgDeviceNum.add(SubOrgInfo);
			System.out.println("next:"+i);
		}
